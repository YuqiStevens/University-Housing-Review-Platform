...
const getAllHousings = async () => {
  ...
  const allHousings = await housingsCollection.find({}).toArray();
  return allHousings;
};
...
const addHousing = async (housing) => {
  let adminId = xss(housing.adminId).trim();
  const housingData = {
  ...
  housingData.established_date = currentTime;
...
export { getAllStores, getStoreById, updateCommentofStore, addStore, removeStore, updateStore };
